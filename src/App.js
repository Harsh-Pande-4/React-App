...
// Sample data generation based on the selected date
...
const App = () => {
  const [data, setData] = useState(generateData(12)); // Generate initial data for 12 hours
  const [lowCostNotification, setLowCostNotification] = useState(false);
  const [date, setDate] = useState(new Date().toISOString().split('T')[0]);
  const [device, setDevice] = useState('Select Device');

  // Update data whenever the date changes
  useEffect(() => {
    setData(generateData(12)); // Regenerate data for 12 hours when date changes
  }, [date]);
